...
function W3_ldap_authenticate($uid,$password) {

        # Must automatically reject if full
        # credentials were not passed.
        if (!$uid || !$password) {
                return array(1,"No credentials filled. Please enter the Intranet ID and the Password !!");
        }

        #       my $dn = "uid=$user, ou=bluepages, o=ibm.com?mail?sub?(&(uid=*))";

        $ldap_conn = ldap_connect("bluepages.ibm.com");

        # First, bind anonymously
        $ldap_bind = ldap_bind($ldap_conn);
        if (!$ldap_bind) {
                return array(2,"Error to Authenticate (".ldap_error($ldap_conn).")");
        }

        $ldap_base = "ou=bluepages, o=ibm.com";
        $ldap_filter = "mail=".$uid;
        $attributes=array('dn','uid','sn');

        # Now search for $uid in the directory
        $ldap_search = ldap_search( $ldap_conn, $ldap_base, $ldap_filter, $attributes);

        if (!ldap_count_entries($ldap_conn,$ldap_search)) {
                return array(3,"Invalid Intranet ID !!.");
        }

        $ldap_register = ldap_get_entries($ldap_conn,$ldap_search);

        $dn = $ldap_register[0]['dn'];

        # Bind to the server using the DN
        # found and the password passed to the
        # function.
        $ldap_bind_user = ldap_bind($ldap_conn, $dn, "$password");
        if (!$ldap_bind_user)
        {
...
function W3_ldapCheckUser($uid) {

        # Must automatically reject if full
        # credentials were not passed.
        if (!$uid) {
                return array(1,"No credentials filled. Please enter the Intranet ID !!");
        }
